...
- After final install success output, install prompts `Would you like to enable hourly automatic updates for the TLTG configs?` when input is interactive.
...
- `install.sh --yes` suppresses interactive legacy rollback confirmation, install confirmation, restart confirmation, and auto-update setup prompts; preflight checks still run. If legacy manually-copied optimized configs are detected, `--yes` backs up and restores stock-managed configs before install. If QIDI Box enablement or tool-slot correction is detected, `--yes` applies the saved-variable change without prompting.
