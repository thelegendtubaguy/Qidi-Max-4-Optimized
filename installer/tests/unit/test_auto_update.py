...
class AutoUpdateTests(unittest.TestCase):
    ...
    def test_enable_auto_updates_installs_systemd_units_through_sudo(self):
        ...
        self.assertEqual(json.loads(state_path(paths).read_text())["latest_checksum"], checksum)
        self.assertIn(["sudo", "-v"], commands)
        self.assertIn(["sudo", "systemctl", "daemon-reload"], commands)
        self.assertIn(["sudo", "systemctl", "enable", "--now", TIMER_NAME], commands)
        install_targets = [command[-1] for command in commands if command[:3] == ["sudo", "install", "-m"]]
